(function() {
  ...
  CoreWidgetAPI = (function(_super) {
    ...
    CoreWidgetAPI.prototype.prime = function(elements, action, event) {
      var target, trigger;
      if (event == null) event = 'mousedown';
      for (trigger in elements) {
        target = elements[trigger];
        return trigger.addEventListener(event, action(target, false));
      }
    };

    CoreWidgetAPI.prototype.unprime = function(elements, event) {
      var trigger, _i, _len;
      if (event == null) event = 'mousedown';
      for (_i = 0, _len = elements.length; _i < _len; _i++) {
        trigger = elements[_i];
        return trigger.removeEventListener(event);
      }
    };
  ...
  CoreWidget = (function(_super) {
    ...
    CoreWidget.util = {
      bind: function(element, event, fn, prop) {
      ...
      get: function(query, node) {
        if (node == null) node = document;
        return node.getElementById(query) || node.getElementsByClassName(query) || node.getElementsByTagName(query) || false;
      },
      ...
      is: function(thing) {
        if ([false, null, NaN, void 0, 0, {}, [], '', 'false', 'False', 'null', 'NaN', 'undefined', '0', 'none', 'None'].indexOf(thing) === -1) {
          return true;
        } else {
          return false;
        }
      ...
      stripScript: function(link) {
        var script;
        if (link.match(/^javascript:(\w\W.)/)) {
          script = RegExp.$1;
    ...
    CoreWidget.animation = {
      duration: 400,
      easing: 'easeInOutExpo',
      complete: null
    };

    CoreWidget.overlay = function(prefix) {
      var overlay;
      if (prefix == null) prefix = 'momentum';
